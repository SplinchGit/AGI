    ...
    def _display_memories(self, memories: list):
        ...
        for mem in memories[:5]:  # Show first 5
            ...
            if mem_type == "conversation":
                ...
                print(f"  >à [{mem['timestamp']}] {mem['source']}: {mem['content'][:80]}... (importance: {mem['importance']})")
            elif mem_type == "knowledge":
                print(f"  =Ú [{mem['category']}] {mem['fact'][:80]}... (confidence: {mem['confidence']})")
